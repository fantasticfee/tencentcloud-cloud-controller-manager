...
func (c *HostClient) Watch(cxt context.Context, l model.ListInterface, revision string) (api.WatchInterface, error) {
	var rev int64
	if len(revision) != 0 {
		var err error
...
func (wc *watcher) watchLoop() {
	...
	for wres := range wch {
		...
		for _, e := range wres.Events {
			...
			if k := model.KeyFromDefaultPath(string(e.Kv.Key)); k != nil {
			...
			apiEvent := &api.WatchEvent{
				Old:  old,
				New:  new,
				Type: eventType,
			}
			wc.sendEvent(apiEvent)
